...
const PresensiHarian = () => {
  ...
  const handleExportPDF = async () => {
    const element = document.getElementById('monthly-report-document');
    if (!element) return;
    setIsExporting(true);
    try {
      ...
      pdf.save(`Rekapitulasi_Kehadiran_Pegawai_${new Date().getTime()}.pdf`);
    ...
    >
        ...
        <div className="tp-content-area monthly-view">
          <div className="glass-card table-card">
            <div className="monthly-header">
              <h3>Rekapitulasi</h3>
              <button className="btn-export-csv icon-only" onClick={() => setShowPreview(true)} title="Unduh">
                <FileDown size={20} />
              </button>
            </div>
            <div className="table-responsive">
      ...
      {showPreview && (
        <div className="modal-overlay" onClick={() => setShowPreview(false)}>
          <div className="modal-content report-preview-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '800px', width: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className="modal-header" style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid #e2e8f0' }}>
              <h3 style={{ margin: 0 }}>Pratinjau Cetak / PDF</h3>
              <button className="btn-close-modal" onClick={() => setShowPreview(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                <X size={20} />
              </button>
            </div>
            
            <div className="official-document" id="monthly-report-document" style={{ padding: '2rem', background: '#fff', color: '#000' }}>
              <div className="doc-header" style={{ textAlign: 'center', marginBottom: '1.5rem', borderBottom: '3px double #000', paddingBottom: '1rem' }}>
                <h2 style={{ margin: '0 0 0.5rem 0', fontSize: '1.5rem' }}>{masterData?.org?.name || 'MADRASAH ALIYAH HUB'}</h2>
                <p style={{ margin: 0, fontSize: '0.9rem' }}>{masterData?.org?.address || 'Jl. Pendidikan No. 45'}</p>
              </div>

              <h3 style={{ textAlign: 'center', marginBottom: '1.5rem', fontSize: '1.1rem', textDecoration: 'underline' }}>LAPORAN REKAPITULASI KEHADIRAN PEGAWAI</h3>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2.5rem', fontSize: '0.85rem' }}>
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    ...
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'left' }}>Nama Pegawai</th>
                    <th style={{ border: '1px solid #000', padding: '8px' }}>Hadir</th>
                    <th style={{ border: '1px solid #000', padding: '8px' }}>Terlambat</th>
                    <th style={{ border: '1px solid #000', padding: '8px' }}>Sakit</th>
                    <th style={{ border: '1px solid #000', padding: '8px' }}>Izin</th>
                    <th style={{ border: '1px solid #000', padding: '8px' }}>Alpa</th>
                  ...
                  {teachers.map((t, idx) => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td style={{ border: '1px solid #000', padding: '8px' }}>{t.name}</td>
                        <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>{hadir}</td>
                        <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>{telat}</td>
                        <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>{sakit}</td>
                        <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>{izin}</td>
                        <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>{alpa}</td>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ textAlign: 'center', width: '250px' }}>
                  <p style={{ margin: '0 0 0.5rem 0' }}>Mengetahui,</p>
                  <p style={{ margin: '0 0 4rem 0' }}>Kepala Madrasah</p>
                  <p style={{ margin: 0 }}><strong>( {masterData?.org?.principal || '............................'} )</strong></p>
                </div>
              </div>
            </div>

            <div className="modal-footer" style={{ padding: '1rem', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: '10px', background: '#f8fafc' }}>
              <button className="btn btn-ghost" onClick={() => setShowPreview(false)} disabled={isExporting} style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid #e2e8f0', cursor: 'pointer' }}>Batal</button>
              <button className="btn btn-primary" onClick={handleExportPDF} disabled={isExporting} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
